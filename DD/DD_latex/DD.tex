...
\begin{tabularx}{\textwidth}{llX}
    ...
    \midrule
    4-Feb-2024 & 2.0 & Altered logical design of database, leading to revised diagrams. Modified URL structure of some endpoints to comply with Spring standards. \\
    \bottomrule
\end{tabularx}
...
\subsection*{Student gets the token to set up the GitHub Action for a battle}

The diagram represented down below shows the process of a Student getting the token to setup the Github action to automatically notify CKB of a push in the repository. Upon successful authentication, the BattleManager communicates with the Model to validate the student's enrollment in the battle and obtain the battle's submission deadline to produce the token, then the BattleManager uses the AuthenticationManager to generate and return to the webapp a valid token.

\begin{figure}[H]
\centering
\includegraphics[scale = 0.33]{Images/diagrams/sequences/getGitHubToken.png}\\
\caption{A Student gets the token for a battle to setup the GitHub Action}
\end{figure}

\subsection*{Educator sees the tournaments they can manage}

The diagram represented down below shows the process of a User visualizing Tournaments filtering by state. Upon successful authentication, the TournamentManager communicates with the Model to retrieve the Tournaments in the given state.

\begin{figure}[H]
\centering
\includegraphics[scale = 0.33]{Images/diagrams/sequences/getTournamentsByState.png}\\
\caption{A User sees the tournaments by state }
\end{figure}

\subsection*{User sees the tournaments by state}

The diagram represented down below shows the process of an Educator visualizing Tournaments he can manage. The TournamentsManager communicates with the Model to retrieve created Tournaments based on the Educator's ID. This triggers a database query executed by the EntityManager. Upon receiving the results, the TournamentsManager sends the list of Tournaments back to the WebApp.
...
\subsection*{Educator sees battles in a tournament}
...
\subsection*{Student sees the battles he is enrolled in within a tournament}
...
\caption{Student sees the battles he is enrolled in within a tournament}
\end{figure}

\subsection*{Student accepts the invite in a group for a battle}

The diagram represented down below shows the process of a Student accepting the invite for a Battle of a tournament he is enrolled in. It begins as the Student initiates a request to accept the invite, using the token recieved as part of the notification. Upon successful token validation, the BattleManager accepts the Battle invite, and if enough students have accepted the invite the group is automatically enrolled in the battle. Either an invalid token, or an already accepted invite triggers error handling, resulting in a null response. In a successful scenario, the user is redirected to the Homepage.

\begin{figure}[H]
\centering
\includegraphics[scale = 0.33]{Images/diagrams/sequences/acceptInvite.png}\\
\caption{Student accepts an invite }
\end{figure}

\subsection*{Educator sees the battles of a tournament he manages}

The diagram represented down below shows the process of an Educator visualizing the list of Battles within the context of a Tournament that he can manage. It begins as the WebApp initiates a request for the Battles, providing the Tournament ID. Upon successful authentication, the BattleManager retrieves the Battles from the Model based on the provided Tournament ID. An invalid Tournament ID triggers error handling, resulting in a null response. In a successful scenario, the Battles flow from the EntityManager to the Model, concluding with the BattleManager transmitting the submissions to the WebApp.

\begin{figure}[H]
\centering
\includegraphics[scale = 0.33]{Images/diagrams/sequences/getCreatedBattles.png}\\
\caption{An Educator sees the battles of a tournament he manages}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[angle=90, scale = 0.5]{Images/diagrams/ComponentSystem.png}\\
...
\begin{description}
    \item[200 Response:] tournaments: Array of Objects
    \item[401 Response:] Unauthorized
\end{description}
...
\begin{description}
    \item[Request Parameters:] tournamentId: int
    \item[200 Response:] "Enrollment successful." : String
    \item[400 Response:] error : Object
    \item[401 Response:] Unauthorized
\end{description}
...
\subsubsection{POST api/battles/\{battleId\}/enroll}
\begin{description}
    \item[Request Parameter:] BattlesId: int, InvitedUsernames: String
    \item[200 Response:] "Enrolled successfully" : String
    \item[400 Response:] error : Object
    \item[401 Response:] Unauthorized
\end{description}
...
\begin{description}
    ...
    \item[200 Response:] success : Object
    \item[400 Response:] error : Object
    \item[401 Response:] Unauthorized
\end{description}
...
\begin{description}
    ...
    \item[200 Response:] score : Object
    \item[400 Response:] error : Object
    \item[401 Response:] Unauthorized
\end{description}
...
\subsubsection{PUT api/profile/\{profileId\}}
\begin{description}
    \item[Request Parameters:] profileId: int
    \item[200 Response:] profiles: Array of Objects
    \item[400 Response:] validationError : Object
    \item[401 Response:] Unauthorized
\end{description}
